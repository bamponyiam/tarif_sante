...
$data['garanty'] = str_replace(',','',$_POST['garanty']);
...
$product->updatePlanLevel($data);
/*$product->deletePremiumByLevel($_POST['id_level']);
...
for($i=0;$i<=83;$i++){
	...
	if($_POST['premium_'.$i] != ""){
		$data_p = array();
		$data_p['id_level'] = $_POST['id_level'];
		$data_p['age'] = $i;
...
$id_product = $product->getProductFromLevelId($_POST['id_level']);

header("Location:product-details?id=".$id_product['id_product']);
