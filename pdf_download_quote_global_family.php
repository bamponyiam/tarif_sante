...
$content = file_get_contents("template/template_quote_global.html");

$french = array('Janvier', 'Février', 'Mars', 'Avril', 'Mai', 'Juin', 'Juillet', 'Août', 'septembre', 'Octobre', 'Novembre', 'Décembre');
$month = $french[date('n')-1];

$content = str_replace("%month-year%", $month.' '.date('Y'), $content);
...
$content = str_replace("%type%", 'FAMILLE', $content);
if($quote['profession'] == ""){
...
if($app[0]['date_of_birth'] != ""){
	$content = str_replace("%1st_dob%", '('.$app[0]['date_of_birth'].') - '.$app[0]['age'].' ans', $content);
}else{
...
if($app[1]['type_applicant'] == "2ND"){
	...
	if($app[1]['date_of_birth'] != ""){
		$content = str_replace("%2nd_dob%", '('.$app[1]['date_of_birth'].') - '.$app[1]['age'].' ans', $content);
	}else{
			$content = str_replace("%2nd_dob%", $app[1]['age'].' ans', $content);
	}
}else{
...
if(sizeof($all_1) > 0){
	...
	$module_1 = "<h3 style='text-align:center;'>RECAPITULATIF DE L’OFFRE</h3>";
	
	$indication .= '<li>Des garanties frais de santé basiques et limitées à la couverture des frais d\'hospitalisation à 100% des frais réels <strong>/ MODULE 1</strong></li>';
	if(sizeof($all_2) > 0){
		$indication .= '<li>Avec extensions aux soins externes <strong>/ MODULE 2</strong> </li>';
		$module_1 = "<h3 style='text-align:center;'>LES RECAPITULATIFS DE L’OFFRE</h3>";
		if(sizeof($all_3) > 0){
			$indication .= '<li>En formule complète avec Optique et Dentaire <strong>/ MODULE 3</strong></li>';
		}
	}else{
		if(sizeof($all_3) > 0){
			$indication .= '<li>Avec extensions aux soins externes <strong>/ MODULE 2</strong></li>';
...
$module_1 .= "<h5 style='text-align:center;'>(Tarification Annuelle – Tarif exprimé en %currency%)</h5>";
...
if(sizeof($all_1) > 0){
	...
	if($app[0]['date_of_birth'] != ""){
		...
		if( $age_cy != $age_ca){
			$module_1 .= '<td width="15%">Pax '.$age_ca.' - '.$age_cy.' ans</td>%age_1st%';
		}else{
			$module_1 .= '<td width="15%">Pax '.$app[0]['age'].' ans</td>%age_1st%';
		}
	}else{
		$module_1 .= '<td width="15%">Pax '.$app[0]['age'].' ans</td>%age_1st%';
	}
	...
	if($app[1]['type_applicant'] == "2ND"){
		...
		if($app[1]['date_of_birth'] != ""){
			...
			if( $age_cy != $age_ca){
				$module_1 .= '<td width="15%">Pax '.$age_ca.' - '.$age_cy.' ans</td>%age_2nd%';
			}else{
				$module_1 .= '<td width="15%">Pax '.$app[1]['age'].' ans</td>%age_2nd%';
			}
		}else{
			$module_1 .= '<td width="15%">Pax '.$app[1]['age'].' ans</td>%age_2nd%';
		}
	...
	if($nb_children > 0){
		if(isset($age_2nd)){
			...
			if($app[2]['date_of_birth'] != ""){
				...
				if( $age_cy != $age_ca){
					$module_1 .= '<td width="15%">Enfant '.$age_ca.' - '.$age_cy.' ans</td>%child1%';
				}else{
					$module_1 .= '<td width="15%">Enfant '.$app[2]['age'].' ans</td>%child1%';
				}
			}else{
				$module_1 .= '<td width="15%">Enfant '.$app[2]['age'].' ans</td>%child1%';
			}
			...
			if(isset($app[3]['age'])){
				...
				if($app[3]['date_of_birth'] != ""){
					...
					if( $age_cy != $age_ca){
						$module_1 .= '<td width="15%">Enfant '.$age_ca.' - '.$age_cy.' ans</td>%child2%';
					}else{
						$module_1 .= '<td width="15%">Enfant '.$app[3]['age'].' ans</td>%child2%';
					}
				}else{
					$module_1 .= '<td width="15%">Enfant '.$app[3]['age'].' ans</td>%child2%';
				}
			...
			if(isset($app[4]['age'])){
				...
				if($app[4]['date_of_birth'] != ""){
					...
					if( $age_cy != $age_ca){
						$module_1 .= '<td width="15%">Enfant '.$age_ca.' - '.$age_cy.' ans</td>%child3%';
					}else{
						$module_1 .= '<td width="15%">Enfant '.$app[4]['age'].' ans</td>%child3%';
					}
				}else{
					$module_1 .= '<td width="15%">Enfant '.$app[4]['age'].' ans</td>%child3%';
				}
			...
			if(isset($app[5]['age'])){
				...
				if($app[5]['date_of_birth'] != ""){
					...
					if( $age_cy != $age_ca){
						$module_1 .= '<td width="15%">Enfant '.$age_ca.' - '.$age_cy.' ans</td>%child4%';
					}else{
						$module_1 .= '<td width="15%">Enfant '.$app[5]['age'].' ans</td>%child4%';
					}
				}else{
					$module_1 .= '<td width="15%">Enfant '.$app[5]['age'].' ans</td>%child4%';
				}
			...
			if($app[1]['date_of_birth'] != ""){
				...
				if( $age_cy != $age_ca){
					...
					$module_1 .= '<td width="15%">Enfant '.$app[1]['age'].' ans</td>%child1%';
				}
			}else{
				$module_1 .= '<td width="15%">Enfant '.$app[1]['age'].' ans</td>%child1%';
			}
			...
			if(isset($app[2]['age'])){
				...
				if($app[2]['date_of_birth'] != ""){
					...
					if( $age_cy != $age_ca){
						...
						$module_1 .= '<td width="15%">Enfant '.$app[2]['age'].' ans</td>%child2%';
					}
				}else{
					$module_1 .= '<td width="15%">Enfant '.$app[2]['age'].' ans</td>%child2%';
				}
			...
			if(isset($app[3]['age'])){
				...
				if($app[3]['date_of_birth'] != ""){
					...
					if( $age_cy != $age_ca){
						...
						$module_1 .= '<td width="15%">Enfant '.$app[3]['age'].' ans</td>%child3%';
					}
				}else{
					$module_1 .= '<td width="15%">Enfant '.$app[3]['age'].' ans</td>%child3%';
				}
			...
			if(isset($app[4]['age'])){
				...
				if($app[4]['date_of_birth'] != ""){
					...
					if( $age_cy != $age_ca){
						...
						$module_1 .= '<td width="15%">Enfant '.$app[4]['age'].' ans</td>%child4%';
					}
				}else{
					$module_1 .= '<td width="15%">Enfant '.$app[4]['age'].' ans</td>%child4%';
				}
	...
	$module_1 .= '<tr>';
	$module_1 .= '<td width="15%">Assistance / Rapatriement Famille.</td>%assistance%';
	$module_1 .= '</tr>';
	
	$module_1 .= '<tr>';
	$module_1 .= '<td width="15%">Frais d’Adhesion</td>%adhesion%';
	$module_1 .= '</tr>';
	
	$module_1 .= '<tr>';
	$module_1 .= '<td width="15%">Remise Famille (Si existe)</td>%discount%';
	$module_1 .= '</tr>';
	
	$module_1 .= '<tr>';
	$module_1 .= '<td width="15%">Frais d’Additionnels</td>%additional%';
	$module_1 .= '</tr>';
	...
	foreach($all_1 as $select){
		...
		if(isset($age_2nd)){
			if($family_pack_active == 1){
				$age_2nd .= '<td style="text-align:center;">Inclus Plan de Famille</td>';
			}else{
		...
		if(isset($child1)){
			if($family_pack_active == 1){
				$child1 .= '<td style="text-align:center;">Inclus Plan de Famille</td>';
			}else{
		...
		if(isset($child2)){
			if($family_pack_active == 1){
				$child2 .= '<td style="text-align:center;">Inclus Plan de Famille</td>';
			}else{
				if($select['children_pack'] != 0){
		...
		if(isset($child3)){
			if($family_pack_active == 1){
				$child3 .= '<td style="text-align:center;">Inclus Plan de Famille</td>';
			}else{
				if($select['children_pack'] != 0){
		...
		if(isset($child4)){
			if($family_pack_active == 1){
				$child4 .= '<td style="text-align:center;">Inclus Plan de Famille</td>';
			}else{
				if($select['children_pack'] != 0){
